 ...
 * @param {boolean} [useCache=true] use the cached device if its there
 * @returns {Promise}
 * @endpoint GET /v1/devices
 */
Spark.prototype.getDevice = function (deviceId, callback, useCache) {
  useCache = useCache === undefined ? true : useCache; // default to true
  var defer = this.createDefer('getDevice', callback);

  // Check if it's in the cache
  var device = useCache ? this.devices[deviceId] : null;
  ...
  if ( !device ) {
    ...
    this.resolveDefer(defer, null, device);
    this.emitAndCallback('getDevice', null, device, callback);
  }
  ...
  var handler = function(err, response, data) {
    ...
    if (Array.isArray(data)) {
      for (var i in data) {
        ...
        var device = new Device(deviceData, this);
        // Update the cache and add it to the list
        this.devices[deviceData.id] = device;
        deviceList.push(device);
      }
    }
    this.resolveDefer(defer, err, deviceList);
    this.emitAndCallback('listDevices', err, deviceList, callback);
  }.bind(this);
 ...
 */
Spark.prototype.compileCode = function(files, options, callback) {
  ...
  this.api.compileCode(files, options, this.accessToken, handler);
